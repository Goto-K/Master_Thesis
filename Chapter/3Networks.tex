...
\newpage
以上より, 飛跡対についての崩壊点の種類は``非結合な飛跡対 (not connected, NC)",``primary vertex (PV)", ``チャーム・フレーバーのsecondary vertex (SVCC)", ``ボトム・フレーバーのsecondary vertex (SVBB)", ``チャーム・フレーバーのtertiary vertex (TVCC)", ``終状態$b\bar{b}$での準崩壊点 (SVBC)", ``これら以外の崩壊点 (Others)"の計$7$つとなる。
