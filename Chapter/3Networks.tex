...
ネットワークの評価は、様々なネットワークとの比較による最良のネットワークの選択と、あるネットワークについての理解の二つの観点によって行う。
この飛跡対についてのネットワークでは、様々なネットワークとの比較については入力変数やネットワークの構造を変え、それらの結果をROC曲線や混合行列を用いることによって評価する。
ネットワークについての理解はt分布型確率的近傍埋め込み法(t-Distributed Stochastic Neighbor Embedding, t-SNE\cite{t-SNEpaper})を用いて、各クラスのデータをどの程度分離できているを可視化することによって行う。\\\\

1. ROC曲線や混合曲線による様々なネットワークの比較\\\\

まず、ネットワークが入力変数の内ある特定の情報のみを使用してしまい怠けてしまうということが考えられるため、入力変数を変えたネットワークのモデルを作成する。
次に本研究において、崩壊点の位置を再構成できているかが非常に重要であるため、正解ラベルの一つであるLCFIPlusによって得られる崩壊点の位置を直接入力したネットワークAとネットワークによって予想された崩壊点の位置を出力層の直前で結合したネットワークBを作成する(図\ref{3-3-3-1PairNetworks})。
また、ネットワークAの出力は分類のみとし、崩壊点の位置の予想は行なっていない。
...
評価するネットワークのモデルを次の表\ref{EvalationModels}に示す。
ここで、図\ref{3-3-1-1PairModel}のネットワーク構造を"ネットワークStandard"と呼んでいる。
...
　\small
 \scalebox{0.85}{
  \begin{tabular}{c c c} \hline
    ...
    モデルA & トラック・パラメータ、共分散行列、電荷、エネルギー &　ネットワークStandard\\
    モデルB & トラック・パラメータ、電荷、エネルギー &　ネットワークStandard\\
    モデルC & トラック・パラメータ、共分散行列、電荷、エネルギー、崩壊点の位置 &　ネットワークA\\
    モデルD & トラック・パラメータ、共分散行列、電荷、エネルギー &　ネットワークB\\\hline
  \end{tabular}
  }
...
モデルA、B、Dに関しては前項で述べた学習手順によってネットワークを学習し、モデルCについては分類問題についての学習回数を4000エポックとした。
それぞれのモデルについて、各クラスの効率とスコアの関係を図\ref{3-3-3-2Efficiency_Curve}に示す。
またROC曲線を図\ref{3-3-3-2ROC_Curve}に示す。

ROC曲線は本来二値分類についての評価を行う為の手法である。
したがって、今回の$7$クラス分類においては数学的に適切ではないが、ここでは直感的な理解のためROC曲線を次のように定義し直した。
\begin{itemize}
 \item 縦軸 : 特定のクラスについての効率
 \item 横軸 : 特定のクラス以外についての効率
\end{itemize}
...
\begin{figure}[htbp]
 \centering
  ...
  \begin{minipage}{1.0\textwidth}
   ...
   \end{minipage}
  \caption{各モデルのROC Curve}
  \label{3-3-3-2ROC_Curve}
　%\end{tabular}
\end{figure}

NC、PV、Othersに関してはどのモデルについても高い効率で識別ができているとわかる。
特に、Othersについては高いスコアに閾値を設けても効率を維持できており、これはROC曲線においても確認できる。

一方SVに関するクラスである、SVCC、SVBB、TVCC、SVBCに関しては分類性能が非常に悪く、分離が困難であると考えられる。
これは図\ref{3-3-3-2ConfusionMatrix}の混合行列の結果からも明らかである。
図\ref{3-3-3-2ConfusionMatrix}では効率と純度についてそれぞれ規格化している。
また、モデルA以外の結果については相対値で示した。
したがって、対角成分が正、もしくは対角成分以外が負ならば性能が向上していると判断できる。
効率についての混合行列ではNC、PV、SVCC、Othersが高い効率で識別できていることがわかる。
ただし、各SV内の分離については殆ど出来ておらず、細かな位置の再構成は出来ていないと考えられる。
純度に関してはNCからの汚染が酷く、PVを除く殆どのクラスにおいて、純度はNCによって埋もれてしまっている。
また、NCからの流入と実際の崩壊点のタネに関する性能は\ref{VFDL:TuneandPerformanceofVFDL}節にて評価する。
...
ROC曲線や混合行列について、モデルAとモデルBについての大きな差は見られなかった。
このことから、飛跡対についてのネットワークは共分散行列を見ていないことがわかる。
モデルAとモデルDについても大きな差は見られず、ネットワークは入力変数からの情報とネットワークによって抽出される崩壊点の位置についての情報を十分に活用していることがわかる。
モデルAとモデルCについて、SVBBやTVCCの分類効率に関して僅かに性能の向上が見られる。
また、純度に関してもTVCCを除き性能の向上を確認できる。
これは各SV内の細かな位置の情報をLCFIPlusの崩壊点の位置を入力することで得られていると考えられる。
一方、ビーム衝突点から比較的遠い位置に存在するOthersを分離できていることから荒い位置の再構成はできているとわかる。\\\\
...
ネットワークの評価は飛跡対についてのネットワークと同様に、ネットワーク間の比較とネットワークの理解の二つの観点で行う。
任意の数の飛跡についてのネットワークでは、まず標準的なLSTMと本研究で構築した独自のネットワークとの比較を損失や正答率を用いて行う。
次に、終状態の違いやPVやSVの違いについて、各データ属性に特化したネットワークと全てのデータを用いて学習した標準的なネットワークについての比較をROC曲線を用いて行う。
最後にネットワークについての理解に関して、任意の数の飛跡についてのネットワークはAttentionを持っているため、Attention Weightを用いて評価する。\\\\

1. 標準的なLSTMと独自のネットワークの比較\\\\

比較のためのネットワークとして、図\ref{3-4-1-1SimpleVLSTM}のような独自のリカレントニューラルネットワーク構造を組み込んだシンプルなネットワークと図\ref{3-4-1-4EncoderDecoderVLSTM}のようなエンコーダー・デコーダーモデルを用いる。
標準的なLSTMは図\ref{3-4-1-1SimpleVLSTM}のようなネットワークについて個々のステップをLSTMに置き換えたネットワークを用いる。
それらのネットワークに関して損失と正答率のエポック毎の変化を図\ref{3-4-3-1LSTMvsVLSTM}に示す。
...
ここでTrue Positiveとは正解を正しく正解だと言えている割合、True Negativeとは不正を正しく不正であると言えている割合である。
ただし、今回はエンコーダー・デコーダーモデルに関してのネットワーク構造を考慮し、これらの値からは崩壊点のタネとなる飛跡対についての正当は取り除いている。
また、ゼロ埋めした飛跡についても同様に取り除いている。
標準的なLSTMは系列への順序を重視しているため学習が安定していないことが分かる。
また、独自のネットワーク同士の比較においても、エンコーダー部からのコンテキストを受け取ることのできるエンコーダー・デコーダーモデルの性能が高くなっている。
以上のことから、標準的なネットワークを単純に使うだけでは性能はかなり低く、独自のネットワーク構造による大幅な性能の改善が確認できた。
更にエンコーダー・デコーダーモデルへ拡張することにより、特にTrue Positiveの大幅な改善を実現できている。\\\\

2. 各データ属性に特化したネットワークとの比較\\\\

データ属性として、終状態が$\rm c\bar{c}$、$\rm b\bar{b}$となるデータ、崩壊点のタネがPV、SV(SVCC・SVBB・TVCC)であるデータに分離し、それぞれのみを使用し作成した訓練データで学習した特化型のネットワークを構築した。
